...
@Component({
  ...
  template: `
    @if (communityId) {
      <button
        type="button"
        class="app-button-secondary mb-4 flex items-center gap-1.5 px-3 py-1.5 text-xs"
        (click)="back.emit()"
      >
...
export class RacesPageComponent implements OnInit {
  ...
  private getRatings$(year: number, raceId?: string) {
    if (this.communityId) {
      return this.ratingsService.getCommunityRatings(year, this.communityId, raceId);
    }

    if (this.onlyMine && !this.authService.isAuthenticated()) {
      return of([] as DriverSeasonRating[]);
    }
  ...
  private loadRace(raceId: string): void {
    ...
    const ratings$ = this.getRatings$(year, raceId);
  ...
  private loadSeasonStandings(): void {
    ...
    const ratings$ = this.getRatings$(year);
